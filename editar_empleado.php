...
// ==========================================
// 1. CEREBRO MULTI-SEDE Y SEGURIDAD
// ==========================================
...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    $_SESSION['mensaje_personal'] = "❌ Solicitud inválida.";
...
// Obtener datos asegurando que pertenece a la sede
$filtro_seguridad = "";
$parametros_emp = [':id_emp' => $id_empleado];

if ($mi_rol !== 'SuperAdmin') {
    $filtro_seguridad = " AND id_nucleo = :id_nuc";
    $parametros_emp[':id_nuc'] = $mi_nucleo;
}

$q_empleado = $conexion->prepare("SELECT * FROM empleados WHERE id_empleado = :id_emp $filtro_seguridad");
$q_empleado->execute($parametros_emp);
$empleado_actual = $q_empleado->fetch(PDO::FETCH_ASSOC);

if (!$empleado_actual) {
    $_SESSION['mensaje_personal'] = "❌ Empleado no encontrado o no tienes permisos.";
    $_SESSION['tipo_mensaje_personal'] = "error";
    header("Location: index.php");
    exit();
}
...
// ==========================================
// 2. PROCESAR LA ACTUALIZACIÓN (UPDATE)
// ==========================================
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $nucleo_asignado = ($mi_rol === 'SuperAdmin') ? $_POST['id_nucleo'] : $empleado_actual['id_nucleo'];

    // Verificar si la NUEVA cédula o ID Biométrico ya los tiene OTRA persona
    $check = $conexion->prepare("SELECT COUNT(*) FROM empleados WHERE (cedula = :cedula OR anviz_id = :anviz) AND id_empleado != :id_emp");
    $check->execute([':cedula' => $cedula, ':anviz' => $anviz_id, ':id_emp' => $id_empleado]);
    
    if ($check->fetchColumn() > 0) {
        $mensaje = "❌ Error: La Cédula o el ID Biométrico pertenecen a otro empleado.";
        $tipo_mensaje = "error";
    } else {
        $sql_update = "UPDATE empleados SET 
                       cedula = :ced, nombres = :nom, apellidos = :ape, anviz_id = :anv, 
                       id_departamento = :dep, cargo = :car, modalidad = :mod, contrato = :con, id_nucleo = :nuc
        ...
        if ($exito) {
            $_SESSION['mensaje_personal'] = "✅ ¡Datos del empleado actualizados exitosamente!";
            $_SESSION['tipo_mensaje_personal'] = "exito";
            header("Location: index.php");
            exit();
        } else {
            $mensaje = "❌ Error interno al actualizar la base de datos.";
            $tipo_mensaje = "error";
        }
    }
}
...
<head>
    ...
    <title>SIBIA - Editar Empleado</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-input:focus, .form-select:focus { outline: none; border-color: #3498db; box-shadow: 0 0 5px rgba(52, 152, 219, 0.3); }
        ...
        .btn-guardar { background: linear-gradient(to bottom, #3498db, #2980b9); color: white; }
        .btn-guardar:hover { background: linear-gradient(to bottom, #2980b9, #1c6ea4); transform: translateY(-2px); }
        ...
        <main class="main-content">
            ...
            <section class="content-area">
                ...
                <div class="record-section">
                    <h1 class="record-title">✏️ Modificar Datos del Empleado</h1>
                    ...
                    <form method="POST" action="">
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="cedula" class="form-input" value="<?= htmlspecialchars($empleado_actual['cedula']) ?>" required>
                            </div>

                            <div class="form-group">
                                <label><span>*</span> ID Biométrico (Anviz ID):</label>
                                <input type="number" name="anviz_id" class="form-input" value="<?= htmlspecialchars($empleado_actual['anviz_id']) ?>" required>
                            </div>

                            <div class="form-group">
                                <label><span>*</span> Nombres:</label>
                                <input type="text" name="nombres" class="form-input" value="<?= htmlspecialchars($empleado_actual['nombres']) ?>" required>
                            </div>

                            <div class="form-group">
                                <label><span>*</span> Apellidos:</label>
                                <input type="text" name="apellidos" class="form-input" value="<?= htmlspecialchars($empleado_actual['apellidos']) ?>" required>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_departamento" class="form-select" required>
                                    <?php foreach($departamentos as $d): ?>
                                        <option value="<?= $d['id_departamento'] ?>" <?= ($empleado_actual['id_departamento'] == $d['id_departamento']) ? 'selected' : '' ?>>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="cargo" class="form-input" value="<?= htmlspecialchars($empleado_actual['cargo']) ?>" required>
                            </div>

                            <div class="form-group">
                                <label><span>*</span> Modalidad Académica/Laboral:</label>
                                <select name="modalidad" class="form-select" required>
                                    <option value="Diurno" <?= ($empleado_actual['modalidad'] == 'Diurno') ? 'selected' : '' ?>>Diurno (Lunes a Viernes)</option>
                                    <option value="Nocturno" <?= ($empleado_actual['modalidad'] == 'Nocturno') ? 'selected' : '' ?>>Nocturno (Fines de Semana)</option>
                                    <option value="Ambos" <?= ($empleado_actual['modalidad'] == 'Ambos') ? 'selected' : '' ?>>Ambas Modalidades</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="contrato" class="form-select" required>
                                    <option value="Fijo" <?= ($empleado_actual['contrato'] == 'Fijo') ? 'selected' : '' ?>>Fijo / Titular</option>
                                    <option value="Contratado" <?= ($empleado_actual['contrato'] == 'Contratado') ? 'selected' : '' ?>>Contratado</option>
                                    <option value="Suplente" <?= ($empleado_actual['contrato'] == 'Suplente') ? 'selected' : '' ?>>Suplente</option>
                                    <option value="Obrero" <?= ($empleado_actual['contrato'] == 'Obrero') ? 'selected' : '' ?>>Obrero</option>
                                </select>
                            </div>

                            <?php if($mi_rol === 'SuperAdmin'): ?>
                            <div class="form-group form-full-width" style="background: #eef2f5; padding: 15px; border-left: 4px solid #3498db; border-radius: 4px;">
                                <label style="color: #1e5799;"><span>*</span> [Acceso Nacional] Transferir de Sede:</label>
                                <select name="id_nucleo" class="form-select" required style="border-color: #3498db;">
                                    <?php foreach($nucleos as $n): ?>
                                        <option value="<?= $n['id_nucleo'] ?>" <?= ($empleado_actual['id_nucleo'] == $n['id_nucleo']) ? 'selected' : '' ?>>
                                            <?= htmlspecialchars($n['nombre_nucleo']) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        ...
                        <div class="action-buttons">
                            <a href="index.php" class="btn btn-volver">Cancelar y Volver</a>
